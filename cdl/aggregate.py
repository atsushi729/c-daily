...
from cdl.constants import DEFAULT_LOG_DIR
from cdl.session_reader import compute_project_stats, load_jsonl
...
def generate(target_date: str, log_base: Path) -> Path:
    """Generate Markdown for target_date and return the output path."""
    raw_dir = log_base / "raw"
    records = load_jsonl(raw_dir / f"{target_date}.jsonl")
    out = log_base / f"{target_date}.md"
    log_base.mkdir(parents=True, exist_ok=True)
    out.write_text(build_md(target_date, records), encoding="utf-8")
